...
- GitHub Actions run #27（`1fe4bcb`）的 Node、Android 单测/Debug 构建和 signer fixture 通过，但真实 APK release gate 仍报 SHA-256 行未匹配；限长诊断提交到 #28 后，确认 Ubuntu `apksigner` 使用 `V2 Signer: certificate ...` 标签，并将 SHA-256 值放在下一行。解析器现兼容 legacy `Signer #1` 与 V2 新格式，测试覆盖新标签和换行值；本地 PS7 解析测试及真实 APK 门禁通过，需由下一次 Actions 验证 runner 格式。
