...
- GitHub Actions run #27（`1fe4bcb`）的 Node、Android 单测/Debug 构建和 signer fixture 通过，但真实 APK release gate 仍报 SHA-256 行未匹配。用 `gh run view --log-failed` 取到的日志只有解析器异常，没有原始输出；当前新增限长、清理控制字符的 signer 相关行诊断，并为 malformed-line 错误信息补了测试。下一提交仍需 CI 取回真实行格式后才能确认修复；正式 keystore 仍等待可保护的离线恢复介质。
