...
- `scripts/verify_release_gates.ps1` 的签名元数据解析兼容旧版 `Signer #1` 与新版 `V2 Signer` 标签、单行/换行指纹、冒号/空格分隔、CRLF 和 ANSI 着色。Actions run #28 的诊断确认了 Ubuntu 新版输出格式；该格式现有针对性 fixture，修复后的真实 runner 结果待下一运行确认。
